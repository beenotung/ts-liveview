...
export async function up(knex: Knex): Promise<void> {
  await knex.raw('alter table `verification_code` add column `revoke_time` integer null')
  ...
  if (!(await knex.schema.hasTable('verification_attempt'))) {
    await knex.schema.createTable('verification_attempt', table => {
      ...
      table.integer('match_id').unsigned().nullable().references('verification_code.id')
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.raw('alter table `verification_code` drop column `revoke_time`')
  {
    ...
    await knex.raw("alter table `verification_code` add column `passcode` text null")
    for (let row of rows) {
