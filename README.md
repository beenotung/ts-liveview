...
- PWA <span id="pwa"></span>: Working setup included.
  - **Manifests** — Generated by [server/app/manifest.ts](./server/app/manifest.ts) into [public/pwa/](./public/pwa/). Edit manifest.ts for names, orientation, colors, icon paths; colors also fill the HTML theme meta.
  - **Icons** — replace icon files in the `public` directory: `icon-192.png`, `icon-512.png`, `favicon.ico`, `apple-touch-icon.png`.
